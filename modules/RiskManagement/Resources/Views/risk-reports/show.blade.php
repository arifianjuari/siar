            ...
            <div class="modal fade" id="createActivityModal" tabindex="-1" aria-labelledby="createActivityModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form action="{{ route('modules.risk-management.risk-reports.create-activity', $riskReport->id) }}" method="POST">
                                @csrf
                                <div class="row">
                                    <div class="col-md-12 mb-3">
                                        <label for="title" class="form-label">Judul Kegiatan <span class="text-danger">*</span></label>
                                        <input type="text" class="form-control" id="title" name="title" value="{{ old('title', 'Tindak Lanjut Risiko: ' . $riskReport->document_title) }}" required>
                                    </div>
                        ...
                        <div class="card mt-3">
                            ...
                            <div class="card-body">
                                ...
                                @else
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div>
                                            <strong>{{ $riskReport->document_title }}</strong><br>
                                            <small class="text-muted">No: {{ $riskReport->document_number }}</small>
                                            <div>
                                                <a href="{{ asset('storage/' . $riskReport->file_path) }}" class="text-primary" target="_blank">
                                                    <i class="fas fa-paperclip me-1"></i> Lihat File
                                                </a>
                                            </div>
                                        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (linkActivityForm) {
            linkActivityForm.addEventListener('submit', function(e) {
                ...
                // Kirim request dengan fetch API
                fetch('{{ route('modules.risk-management.risk-reports.link-activity', $riskReport->id) }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken,
        ...
        if (confirmUnlinkBtn) {
            confirmUnlinkBtn.addEventListener('click', function() {
                ...
                // Kirim request dengan fetch API
                fetch('{{ route('modules.risk-management.risk-reports.unlink-activity', $riskReport->id) }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken,
                        'Accept': 'application/json',
