...
class SPOController extends Controller
{
    ...
    public function create()
    {
        ...
        $documentTypes = config('spo-management.document_types');
        $confidentialityLevels = config('spo-management.confidentiality_levels');

        // Default values
        $defaultValues = array_merge(
            config('spo-management.defaults'),
            [
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'document_type' => ['required', Rule::in(array_keys(config('spo-management.document_types')))],
            'document_number' => 'required|string|max:255',
            'document_date' => 'required|date',
            'document_version' => 'required|string|max:10',
            'confidentiality_level' => ['required', Rule::in(array_keys(config('spo-management.confidentiality_levels')))],
        ...
        try {
            ...
            return redirect()->route('spo.index')
    ...
    public function show(SPO $spo)
    {
        ...
        $linkedUnits = collect([]);
        if ($spo->linked_unit) {
            $linkedUnitIds = json_decode($spo->linked_unit, true);
    ...
    public function update(Request $request, SPO $spo)
    {
        ...
        $request->validate([
            ...
            'file_url' => 'nullable|url|max:255',
            'review_cycle_months' => 'required|integer|min:1|max:60',
            'status_validasi' => ['required', Rule::in(array_keys(config('spo-management.status_validasi')))],
            'definition' => 'nullable|string',
            'purpose' => 'nullable|string',
            'policy' => 'nullable|string',
            'procedure' => 'nullable|string',
    ...
    public function generateQr(SPO $spo)
    {
        ...
        $qrContent .= "Tgl. Revisi: " . $spo->updated_at->format('d/m/Y') . "\n";
        $qrContent .= "Unit: " . ($spo->workUnit ? $spo->workUnit->unit_name : 'N/A') . "\n";
        $qrContent .= "URL: " . route('spo.show', $spo);
