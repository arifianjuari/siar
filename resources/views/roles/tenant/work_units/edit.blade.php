        ...
        <div class="card-body">
            <form action="{{ route('tenant.work-units.update', $workUnit) }}" method="POST">
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" class="form-control @error('unit_name') is-invalid @enderror" id="unit_name" name="unit_name" value="{{ old('unit_name', $workUnit->unit_name) }}" required autofocus>
                        @error('unit_name')
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" class="form-control @error('unit_code') is-invalid @enderror" id="unit_code" name="unit_code" value="{{ old('unit_code', $workUnit->unit_code) }}">
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <select class="form-select @error('unit_type') is-invalid @enderror" id="unit_type" name="unit_type" required>
                            ...
                            <option value="medical" {{ old('unit_type', $workUnit->unit_type) == 'medical' ? 'selected' : '' }}>Medical</option>
                            <option value="non-medical" {{ old('unit_type', $workUnit->unit_type) == 'non-medical' ? 'selected' : '' }}>Non-Medical</option>
                            <option value="supporting" {{ old('unit_type', $workUnit->unit_type) == 'supporting' ? 'selected' : '' }}>Supporting</option>
                        </select>
                        @error('unit_type')
                            <div class="invalid-feedback">
                                {{ $message }}
                    ...
                    <div class="col-sm-10">
                        <select class="form-select @error('head_of_unit_id') is-invalid @enderror" id="head_of_unit_id" name="head_of_unit_id" required>
                            ...
                            @foreach($users ?? [] as $user)
                                <option value="{{ $user->id }}" {{ old('head_of_unit_id', $workUnit->head_of_unit_id) == $user->id ? 'selected' : '' }}>
                                    {{ $user->name }}
                                </option>
                            @endforeach
                        </select>
